...
class Main(http.Controller):
    ...
    @http.route('/eliminarempates', auth='user', type='json', methods=['POST'])
    def eliminar_empates(self):
        partidos = http.request.env['liga.partido'].sudo().search([('goles_casa', '=', 'goles_fuera')])
        eliminados = len(partidos)
        partidos.unlink()
        return {'partidos_eliminados': eliminados}
